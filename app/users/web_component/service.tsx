...
import { CalendarIcon, BedIcon, PhoneIcon, ClipboardListIcon, AmbulanceIcon, UserIcon, HeartPulseIcon, ActivityIcon, CheckCircleIcon, AlertCircleIcon, ClockIcon, StarIcon, SearchIcon, InfoIcon } from 'lucide-react'
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        {filteredServices.length === 0 && (
          ...
          >
            <p className="text-xl text-gray-600">No services found. Please try a different search term.</p>
          </motion.div>
        )}
        ...
        <Dialog open={!!selectedService} onOpenChange={() => setSelectedService(null)}>
          <DialogContent className="sm:max-w-[550px]">
            <DialogHeader>
              ...
              <DialogDescription className="text-lg">{selectedService?.description}</DialogDescription>
            </DialogHeader>
            <div className="py-4">
              ...
              <div className="mt-4 flex items-center">
                ...
                <div className="flex items-center">
                  {[...Array(5)].map((_, i) => (
                    <StarIcon key={i} className={`h-5 w-5 ${i < Math.floor(selectedService?.rating) ? 'text-yellow-400' : 'text-gray-300'} fill-current`} />
                  ))}
                  <span className="ml-2 text-sm text-gray-600">{selectedService?.rating}/5</span>
                </div>
              </div>
            </div>
            <DialogFooter>
      ...
      <Card className="h-full flex flex-col overflow-hidden border-2 border-transparent hover:border-blue-500 transition-colors duration-300">
        ...
        <CardContent className="flex-grow">
          ...
          <div className="flex items-center justify-center mb-4">
            {[...Array(5)].map((_, i) => (
              <StarIcon key={i} className={`h-5 w-5 ${i < Math.floor(service.rating) ? 'text-yellow-400' : 'text-gray-300'} fill-current`} />
            ))}
            <span className="ml-2 text-sm text-gray-600">{service.rating}/5</span>
          </div>
        </CardContent>
        <CardFooter className="flex justify-between items-center">
          <Button onClick={onSelect} variant="default">Learn More</Button>
          <TooltipProvider>
            <Tooltip>
